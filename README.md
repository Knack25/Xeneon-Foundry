...
- [Host updater foundation](foundry-edge-updater/README.md): signed-manifest validation, trusted-source retrieval, compatibility policy and the maintenance safety gate; installation is not enabled.
...
The [coordinated update design](docs/superpowers/specs/2026-09-20-foundry-automatic-updates-design.md) specifies signed compatible releases and installation only while users are disconnected and Edge actions are idle. The completed foundation validates signed, origin-constrained manifests, evaluates compatibility, reports component identity and enforces a five-minute safety gate from fresh Foundry presence, explicit Edge activity and durable unresolved commands. **Automatic updates are not implemented yet:** there is no configured stable source/key, release polling, host installer, staging, backup/recovery service or update UI, and connector reporting keeps `automaticInstall.enabled` false. The gate cannot download or install anything by itself. Future Foundry release assets belong in this repository, independently of Microsoft widget releases. No production Foundry `manifest` or `download` URL is added until an actual signed test-channel release exists.
