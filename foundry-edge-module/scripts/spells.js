...
export function createSpellControls({getScope}){
 ...
 function recent(actor,userId){prune();return [...casts.values()].filter(c=>c.actorId===actor.id&&c.userId===userId&&(c.consumedItem||actor.items.some(i=>i.id===c.itemId))).map(c=>({castId:c.castId,name:c.name,level:c.level,attack:c.activity.type==='attack',damage:typeof c.activity.rollDamage==='function'&&!!(c.activity.damage?.parts?.length||c.activity.healing),healing:c.activity.type==='heal'}));}
 async function cast(actor,userId,command,message){
  ...
  const input=command.input;const item=[...actor.items].find(i=>i.id===input.itemId&&(command.operation==='activity.use'?['feat','consumable','equipment','tool'].includes(i.type):i.type==='spell'));const activity=item?.system.activities?.get(input.activityId);
  ...
  casts.set(castKey(actor,userId,command.requestId),{castId:command.requestId,userId,actorId:actor.id,itemId:item.id,consumedItem:!actor.items.some(i=>i.id===item.id),messageId:result.message.id,name:item.name,level:(item.system.level??0)+scaling,activity:clone.system.activities.get(activity.id),scope,expires:Date.now()+600000});
 }
 async function roll(actor,userId,operation,input,message){
  prune();const cast=casts.get(castKey(actor,userId,input.castId));
  if(!cast||cast.userId!==userId||cast.actorId!==actor.id||(!cast.consumedItem&&!actor.items.some(i=>i.id===cast.itemId)))throw failure('expired-cast','This cast is no longer available. Use its Foundry chat card.');
